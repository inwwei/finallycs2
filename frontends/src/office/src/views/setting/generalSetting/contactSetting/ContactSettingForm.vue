      ...
      <b-form ref="form">
        <validation-observer ref="Settingform">
          <b-row class="mr-5">
            ...
            <b-col
              v-if="contact_form.ref_id !== null"
              cols="12"
              class="ml-1 mb-2 demo-inline-spacing"
            >
              <b-form-radio
                v-model="contact_form.type"
                name="radios1"
                value="ที่อยู่"
                rules="required"
              >
          ...
          if (result) {
            this.$swal({
              ...
              if (result.isConfirmed) {
                this.$http.post('/api/setting/master/contact/', this.contact_form)
                this.$swal({
                  icon: 'success',
                  ...
                  if (result.isConfirmed) {
                    this.clearform()
                    this.$emit('clickSubmit', this.contact_form)
                  }
                })
              }
            })
        ...
        try {
          ...
          if (result) {
            this.$swal({
              ...
              if (result.isConfirmed) {
                this.$http.put(`/api/setting/master/contact/${this.contact_form.id}`, this.contact_form)
                this.$swal({
                  icon: 'success',
